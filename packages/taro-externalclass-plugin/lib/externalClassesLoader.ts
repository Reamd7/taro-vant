...
function buildBabelTransformOptions() {
  ...
  return {
    ...
    plugins: plugins
      .concat(["preval"])
      .concat(
        process.env.TARO_ENV === "rn" ? [] : functional_1.functionalComponent
      )
      .concat([
...
const cacheMap: Record<string, string | false> = excludeList.reduce(
  (res, val) => {
    res[val] = false;
    return res;
  },
...
module.exports = function(this: loader.LoaderContext, code: string) {
  ...
  try {
    print(
      chalk.green("taro-vant externalClassesLoader 处理中: ") +
        this.resourcePath +
        this.resourceQuery
    );
    ...
    const handlePath = (__path__: string) => {
      ...
      return new Promise<string | false>((resolve, reject) => {
        ...
        myResolver.resolve({}, this.context, __path__, {}, (
          err /*Error*/,
          filepath /*string*/
        ) => {
          if (err) {
          ...
          traverse(ast, {
            ...
            JSXOpeningElement: astPath => {
              ...
              if (t.isJSXIdentifier(JSXIdentifierNode)) {
                    ...
                    attribute.forEach(attr => {
                      ...
                      if (t.isJSXAttribute(attr)) {
                        if (t.isJSXNamespacedName(attr.name)) {
                          console.log(
                            chalk.red(
                              "taro-vant externalClassesLoader: JSX 还不支持namespace:attr 语法"
                            )
                          );
                        } else {
                        ...
                        console.log(
                          chalk.red(
                            "taro-vant externalClassesLoader: Taro 中JSX 不支持 {...Name} 的语法"
                          )
                        );
                      }
                    });
                ...
                console.log(
                  chalk.red(
                    "taro-vant externalClassesLoader: Taro 限制了一个页面只有一个组件"
                  )
                );
              }
            }
          });
          const newCode = transformFromAstSync(
            ast!,
            code,
...
module.exports.pitch = function(this: loader.LoaderContext, __path__: string) {
  ...
  print(
    chalk.greenBright("taro-vant externalClassesLoader pitch: ") + __path__
  );
  ...
  try {
    const ast = parseSync(
      fs.readFileSync(__path__).toString(),
      buildBabelTransformOptions()
    );
    traverse(ast, {
      // NOTE：单独针对taro-vant的组件风格进行模式识别，我不太熟悉ast的处理，所以只能有限于taro-vant的组件了
